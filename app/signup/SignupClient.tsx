...
import { Eye, EyeOff, UserPlus, Gamepad2 } from 'lucide-react';
...
export default function SignupClient() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await refresh();
    showToast(data.role === 'admin' ? '⚡ Admin account created!' : '✅ Account created! Welcome!');
  ...
  const strengthMeta = [
    ...
    { label: 'Good', color: '#f59e0b', bg: 'rgba(245,158,11,0.15)' },
    { label: 'Strong', color: '#22c55e', bg: 'rgba(34,197,94,0.15)' },
  ];
  ...
  return (
    <div style={{ minHeight: '88vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', position: 'relative', overflow: 'hidden' }}>
      ...
      {/* Background orbs */}
      <div style={{ position: 'fixed', top: '-10%', right: '-10%', width: '500px', height: '500px', background: 'radial-gradient(circle, rgba(124,58,237,0.08) 0%, transparent 70%)', pointerEvents: 'none', borderRadius: '50%' }} />
      <div style={{ position: 'fixed', bottom: '-10%', left: '-10%', width: '400px', height: '400px', background: 'radial-gradient(circle, rgba(168,85,247,0.06) 0%, transparent 70%)', pointerEvents: 'none', borderRadius: '50%' }} />
      ...
      <div style={{ width: '100%', maxWidth: '400px', position: 'relative', zIndex: 1 }}>
        <div style={{ background: 'rgba(15,15,28,0.85)', border: '1px solid rgba(124,58,237,0.2)', borderRadius: '20px', padding: '2.5rem', backdropFilter: 'blur(20px)', boxShadow: '0 24px 64px rgba(0,0,0,0.5), 0 0 0 1px rgba(124,58,237,0.1)' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
            ...
            <p style={{ color: '#475569', fontSize: '0.85rem' }}>Create your free account</p>
          </div>

          <form onSubmit={handleSubmit}>
            {/* Email */}
            <div style={{ marginBottom: '1rem' }}>
              <label style={{ display: 'block', color: '#64748b', fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: '0.45rem' }}>Email Address</label>
              <input type="email" required style={inp} value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com"
            ...
            <div style={{ marginBottom: '0.5rem' }}>
              <label style={{ display: 'block', color: '#64748b', fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: '0.45rem' }}>Password</label>
              <div style={{ position: 'relative' }}>
                <input type={showPass ? 'text' : 'password'} required style={{ ...inp, paddingRight: '2.75rem' }}
            ...
            {password.length > 0 && strengthMeta[strength] && (
              <div style={{ marginBottom: '1rem' }}>
                <div style={{ display: 'flex', gap: '3px', marginBottom: '5px' }}>
                  {[1, 2, 3].map(i => (
                    <div key={i} style={{ flex: 1, height: '3px', borderRadius: '2px', transition: 'background 0.3s', background: i <= strength ? strengthMeta[strength]!.color : 'rgba(255,255,255,0.08)' }} />
                  ))}
                </div>
                <span style={{ fontSize: '0.7rem', color: strengthMeta[strength]!.color, fontWeight: 600 }}>
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              <label style={{ display: 'block', color: '#64748b', fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: '0.45rem' }}>Confirm Password</label>
              <input type={showPass ? 'text' : 'password'} required
                style={{ ...inp, borderColor: confirm && confirm !== password ? 'rgba(239,68,68,0.4)' : 'rgba(124,58,237,0.25)' }}
                value={confirm} onChange={e => setConfirm(e.target.value)} placeholder="Re-enter password"
                onFocus={e => { e.target.style.borderColor = 'rgba(168,85,247,0.6)'; e.target.style.background = 'rgba(124,58,237,0.05)'; }}
                onBlur={e => { e.target.style.borderColor = confirm && confirm !== password ? 'rgba(239,68,68,0.4)' : 'rgba(124,58,237,0.25)'; e.target.style.background = 'rgba(255,255,255,0.03)'; }} />
              {confirm && confirm !== password && (
                <p style={{ color: '#f87171', fontSize: '0.72rem', marginTop: '0.3rem' }}>Passwords don&apos;t match</p>
              )}
            </div>

            {/* Error */}
            {error && (
              <div style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.25)', color: '#f87171', padding: '0.7rem 1rem', borderRadius: '10px', fontSize: '0.83rem', marginBottom: '1rem' }}>
                ⚠️ {error}
              </div>
            )}
          ...
          <p style={{ textAlign: 'center', marginTop: '1.5rem', color: '#334155', fontSize: '0.83rem' }}>
            ...
            <Link href="/login" style={{ color: '#a855f7', fontWeight: 700, textDecoration: 'none' }}>Sign in</Link>
          </p>
        </div>
      </div>
